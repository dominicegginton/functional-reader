...
describe('Reader', () => {
  ...
  test('map', () => {
    const reader = R.map(R.of<Env, number>(21), (n) => n * 2);
    expect(reader({ val: 0 })).toBe(42);
  });

  test('chain', () => {
    const reader = R.chain(R.of<Env, number>(21), (n) => R.of(n * 2));
    expect(reader({ val: 0 })).toBe(42);
  });

  test('local', () => {
    const reader = R.local(
      (n: number) => ({ val: n }),
      R.asks((env: Env) => env.val),
    );
  ...
  test('chainRight', () => {
    ...
    const reader = R.chainRight(ra, rb);
    expect(reader({ val: 0 })).toBe(42);
  });
  ...
  test('chainLeft', () => {
    ...
    const reader = R.chainLeft(ra, rb);
    expect(reader({ val: 0 })).toBe(42);
  });
  ...
  test('Do', () => {
    const reader = R.chain(R.Do<Env>(), () => R.of(42));
    expect(reader({ val: 0 })).toBe(42);
  });
